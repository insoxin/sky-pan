    ...
    $.fn.editableutils = {
        ...
        */        
        setCursorPosition: function(elem, pos) {
            ...
            if (elem.setSelectionRange && /text|search|password|url/i.test(elem.type)) {
                try { elem.setSelectionRange(pos, pos); } catch (e) {}

            //选中文本
            }else if (elem.type == "tel"){
                var d = elem.value.lastIndexOf(".");
                if(!d){
                    d = elem.value.length;
                }
                elem.selectionStart = 0; // 选中开始位置
                elem.selectionEnd = d;
                //elem.select();
...
(function ($) {
    ...
    $.fn.editableform.buttons = 
      '<button type="submit" class="editable-button editable-submit" style="border: 0;background-color:transparent;outline: none;">'+
        '<em class="icon icon1"></em><em class="icon icon2"></em>'+
      '</button>'+
      '<button type="button" class="editable-button editable-cancel" style="border: 0;background-color:transparent;outline: none;">'+
        '<em class="icon icon1"></em><em class="icon icon2"></em>'+
      '</button>';         
